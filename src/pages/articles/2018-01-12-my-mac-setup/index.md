...
I never used Alfred until this past summer. Why did I not use it sooner?! *(╯°□°）╯︵ ┻━┻*. The Syntax FM podcast has a great episode on tooling, including Alfred, [Our favourite Productivity Hacks 🔥 — Syntax Podcast 011](https://syntax.fm/show/011/our-favourite-productivity-hacks). You should check it out.
...
* [willfarrell/alfred-encode-decode-workflow: Encoding and decoding a sting into multiple variations.](https://github.com/willfarrell/alfred-encode-decode-workflow). Grab it [here](https://raw.github.com/willfarrell/alfred-encode-decode-workflow/master/encode-decode.alfredworkflow). A super quick way to encode anything. Launch Alfred and type in `encode <div>some markup</div>`, press ENTER and you get `%3Cdiv%3Esome+markup%3C%2Fdiv%3E`. I just used it to so that! Things are getting crazy. To do the reverse use `decode`.
* [ruedap/alfred-font-awesome-workflow: Font Awesome Workflow for Alfred 2 & 3](https://github.com/ruedap/alfred-font-awesome-workflow). Grab it [here](https://github.com/ruedap/alfred-font-awesome-workflow/raw/master/Font-Awesome.alfredworkflow).
...
* [UUIDgen | Packal](http://www.packal.org/workflow/uuidgen). Generate a UUID on the fly and it inserts it into which ever application had the cursor focus before Alfred. `B4CEC447-3D1C-46C2-B1D8-BAC718EA4659` fresh from UUIDGen!
...
* I have a few others I made, but they’re just simple workflows for opening stuff like my Google Music. The workflow is very simple though. It uses an Alfred workflow template to just open a URL. Try making one yourself! I promise it’s not hard.

If you use Alfred and have some workflows that are not listed here, please mention them in the comments. Always looking for new ones to improve my whole dev workflow.
...
* [now.sh](https://now.sh), great for hosting but also great for knocking out some POCs and deploying it. Grab the [Desktop App](https://zeit.co/download) which will also install the CLI. Run `brew cask install now` from the command line to install it.
...
* [FiraCode font](https://github.com/tonsky/FiraCode) for shell and my my favourite editor.
...
* [git aliases](https://git-scm.com/book/en/v2/Git-Basics-Git-Aliases), with mine [here](https://gist.github.com/nickytonline/128d7d164d1eea5daba6753ed0829f41)

## Useful Utilities

* [Unarchiver](https://theunarchiver.com) - run `brew cask install the-unarchiver` from the command line to install it.
* [Amphetamine](https://itunes.apple.com/us/app/amphetamine/id937984704), sometimes you just want to stay awake…
...
* If you’re on a Mac with a Touchbar, map function keys to always be used when in Browsers, VS Code or any other tools you use for dev.

That’s pretty much the round up of what I have on my machine at the moment. I’ll probably get around to writing a script to install all things that can be installed via the command line, but for the time being, other priorities.
